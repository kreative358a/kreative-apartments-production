...
class IssueCreateAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer: IssueSerializer) -> None:
        ...
        # apartment_unit_number =  self.kwargs.get("apartment_unit_number")
        # apartment_building = self.kwargs.get("apartment_building")
        ...
        try:
            ...
            # try:
            #     apartment_check = Apartment.objects.filter(id=apartment_id).first()
            #     apartment_unit_number = apartment_check.unit_number
            #     apartment_building = apartment_check.building
            # except:
            #     raise PermissionDenied(
            #         "You do not have permission to report an issue for this apartment. Because it does not esist"
            #         )                
                
            raise PermissionDenied(
                # f"You do not have permission to report an issue for apartment in building {apartment_building}, no. {apartment_unit_number}. Because it's not yours."
                "You do not have permission to report an issue for this apartment. Because it's not yours"
            )
